...
Nessa API, foram aplicados os princípios da Clean Architecture para estruturar o projeto de forma organizada e escalável. A arquitetura é dividida em camadas, como a camada de apresentação (responsável por receber as requisições HTTP e retornar as respostas), a camada de domínio (onde são definidas as regras de negócio) e a camada de dados (responsável pela interação com o banco de dados ou outras fontes de dados externas).
